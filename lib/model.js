...
export const instanceOf = Clazz => {
  const fn = PropTypes.instanceOf(Clazz);
  if (Clazz) {
    if (typeof Clazz.hash === 'function' && typeof Clazz.register === 'function') {
      ...
      if (registeredClasses[fn._h_] && registeredClasses[fn._h_] !== Clazz) {
        throw new Error(`Attempt to overwrite registeredClass: ${Clazz.name}`);
      }
